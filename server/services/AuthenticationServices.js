...
const verifyDigitalSignature = async (user, publicAddress, signature) => {
  ////////////////////////////////////////////////////
...
const createJWT = async (user, publicAddress) => {
  ////////////////////////////////////////////////////
  // Step 4: Create JWT
  ////////////////////////////////////////////////////
...
const authenticateUser = async (publicAddress, signature) => {
  let error,
    accessToken = null;
  const user = await verifyUser(publicAddress);
  if (!user) {
    return {
      error: `User with publicAddress ${publicAddress} is not found in database`,
    };
  }
  const authen = [verifyDigitalSignature, generateNewNonce, createJWT];
    ...
    if (!resultFunction) {
      switch (i) {
        case 0:
          ...
          break;
        case 1:
          error = 'User is not defined in "Generate a new nonce for the user"';
          break;
        case 2:
